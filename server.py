...
def load_state(conversation_id: str) -> Tuple[str, Optional[str], SupportTicketContext]:
    ...
    return CONVERSATION_STATE.get(
        conversation_id,
        ("triage_agent", None, SupportTicketContext()) # Default state: no previous response_id
    )
...
def save_state(
    ...
    logger.info(f"Saved state for conversation_id: {conversation_id}. Last response ID: {last_response_id}")
# --- End State Store ---
...
async def run_agent_endpoint(request: Request):
    ...
    logger.info(f"[LOAD STATE] For conversation_id: {conversation_id}, loaded agent: {current_agent_name}, \
last_response_id: {last_response_id}")
    ...
    try:
        ...
        new_agent_messages = []
        logger.info(f"Agent result.new_items: {[type(item).__name__ for item in result.new_items]}\n\n")
        logger.info(f"Agent result.last_agent: {getattr(result.last_agent, 'name', repr(result.last_agent))}\n\n")
        logger.info(f"Agent result.last_response_id: {getattr(result, 'last_response_id', None)}\n\n")
        logger.info(f"Agent result.raw_responses: {[item for item in result.raw_responses]}\n\n")
        logger.info(f"Agent result.input: {result.input}\n\n")
        for idx, item in enumerate(result.new_items):
            logger.info(f"  Item {idx}: type={type(item).__name__}, content={getattr(item, 'content', None)}")
            if isinstance(item, MessageOutputItem):
                text_content = ItemHelpers.text_message_output(item)
                logger.info(f"    MessageOutputItem content: {text_content}")
                if text_content:
                    new_agent_messages.append({"role": "assistant", "content": text_content})
            else:
        ...
        if not new_agent_messages:
            ...
            for item in result.new_items:
                if isinstance(item, ToolCallItem) and item.raw_item.name == wandbot_tool.name:
                    wandbot_call_id = item.raw_item.call_id
                    logger.info(f"Found ToolCallItem for {wandbot_tool.name} with call_id: {wandbot_call_id}")
                # Check ToolCallOutputItem's raw_item dictionary for call_id
                elif isinstance(item, ToolCallOutputItem) and item.raw_item['call_id'] == wandbot_call_id:
                    wandbot_output = item.output
                    logger.info(f"Found matching ToolCallOutputItem for {wandbot_tool.name} with output.")
                    break # Found the output, no need to check further

            if wandbot_output:
                logger.info(f"Using output from {wandbot_tool.name} as assistant message.")
                new_agent_messages.append({"role": "assistant", "content": wandbot_output})
            else:
                logger.warning("No MessageOutputItem and no wandbot_tool output found in result!")
        ...
        if handoff_detected:
            ...
            next_agent_name = current_agent_name  # Stay on the current agent unless explicit handoff

        new_response_id = result.last_response_id # Use the property to get the last response ID

        logger.info(f"Agent run finished. Next agent: {next_agent_name}. New Response ID: {new_response_id}. New messages: {len(new_agent_messages)}")
        ...
        logger.info(f"[SAVE STATE] For conversation_id: {conversation_id}, saved agent: {next_agent_name}, new_response_id: {new_response_id}")
        ...
        if new_agent_messages:
            ...
            assistant_message_content = str(raw_content) if raw_content is not None else "Agent returned empty content."
        else:
            agent_error = True
